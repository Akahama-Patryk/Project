...
$data = new Coupon();
$dataCoupon = $data->GetCoupon(null);
?>
<html>
<head>
    <meta charset="utf-8">
                ...
                <main class="px-3 mt-3">
                    <div class='col-md-12'>
                        <table class='table'>
                            ...
                            <?php
                            if ($dataCoupon !== null)
                                foreach ($dataCoupon as $record) :
                                    ?>
                                    <tr>
                                        <td><?= $record['coupon_code'] ?></td>
                                        <td><?= $record['expire_date'] ?></td>
                                        <td><?php if ($record['expire_date'] >= $current_date_filter) $expire_date_status = 'active';
                                            if ($record['expire_date'] < $current_date_filter) $expire_date_status = 'expired' ?> <?= $expire_date_status ?></td>
                                        <form method="get">
                                            <td><a href="dashboard_admin_QR_view?Q_ID=<?= $record['coupon_code']?>">QR Code</a>
                                                </a></td>
                                        </form>
                                    </tr>
                                <?php endforeach; ?>
                    ...
                    <a href="dashboard_admin_coupons_add" class="btn btn-primary align-items-md-end float-right"><i
